...
NSTimeInterval kMAMapSMCalloutViewRepositionDelayForUIScrollView = 1.0/3.0;

#define CALLOUT_DEFAULT_MIN_WIDTH 75 // our image-based background graphics limit us to this minimum width...
#define CALLOUT_DEFAULT_HEIGHT 200 // ...and allow only for this exact height.
...
#define ACCESSORY_MARGIN 9 // the accessory's margin from the edges of our callout view
...
@implementation MAMapSMCalloutView {
    UILabel *titleLabel, *subtitleLabel;
    UIImageView *leftCap, *rightCap, *topAnchor, *bottomAnchor, *leftBackground, *rightBackground;
    MAMapSMCalloutArrowDirection arrowDirection;
    BOOL popupCancelled;
    CGFloat _calloutHeight;
}

@synthesize calloutHeight = _calloutHeight;

@synthesize backgroundImage = _backgroundImage;

- (id)initWithFrame:(CGRect)frame {
    if (self = [super initWithFrame:frame]) {
        _presentAnimation = MAMapSMCalloutAnimationBounce;
        _dismissAnimation = MAMapSMCalloutAnimationFade;
...
- (MAMapSMCalloutBackgroundView *)backgroundView {
    // create our default background on first access only if it's nil, since you might have set your own background anyway.
//    return _backgroundView ?: (_backgroundView = [MAMapSMCalloutImageBackgroundView new]);
    return _backgroundView ?: (_backgroundView = [MAMapSMCalloutDrawnBackgroundView new]);
}
...
- (void)presentCalloutFromRect:(CGRect)rect inLayer:(CALayer *)layer ofView:(UIView *)view constrainedToLayer:(CALayer *)constrainedLayer permittedArrowDirections:(MAMapSMCalloutArrowDirection)arrowDirections animated:(BOOL)animated {
    ...
    MAMapSMCalloutArrowDirection bestDirection = MAMapSMCalloutArrowDirectionDown;
    
    // we'll point it up though if that's the only option you gave us.
    if (arrowDirections == MAMapSMCalloutArrowDirectionUp)
        bestDirection = MAMapSMCalloutArrowDirectionUp;
    ...
    if (arrowDirections == MAMapSMCalloutArrowDirectionAny && topSpace < self.calloutHeight && bottomSpace > topSpace)
        bestDirection = MAMapSMCalloutArrowDirectionUp;
    
    // show the correct anchor based on our decision
    topAnchor.hidden = (bestDirection == MAMapSMCalloutArrowDirectionDown);
    bottomAnchor.hidden = (bestDirection == MAMapSMCalloutArrowDirectionUp);
    ...
    CGFloat anchorY = self.calloutOffset.y + (bestDirection == MAMapSMCalloutArrowDirectionDown ? CGRectGetMinY(rect) : CGRectGetMaxY(rect));
    ...
    CGPoint calloutOrigin = {
        .x = - self.frame.size.width/2 + rect.size.width/2,//calloutX + adjustX,
        .y = bestDirection == MAMapSMCalloutArrowDirectionDown ? (anchorY - self.calloutHeight + BOTTOM_ANCHOR_MARGIN) : anchorY
    };
...
- (BOOL)pointInside:(CGPoint)point withEvent:(UIEvent *)event
{
    ...
    {
        if ([subview isKindOfClass:_subviewClass])
        {
...
- (CAAnimation *)animationWithType:(MAMapSMCalloutAnimation)type presenting:(BOOL)presenting {
    CAAnimation *animation = nil;
    
    if (type == MAMapSMCalloutAnimationBounce) {
    ...
    else if (type == MAMapSMCalloutAnimationFade) {
    ...
    else if (type == MAMapSMCalloutAnimationStretch) {
...
- (void)layoutSubviews {
    ...
    UIImage  *newImage = [_backgroundImage resizableImageWithCapInsets:UIEdgeInsetsMake(5, 10, 10, 5)];
    
    UIGraphicsBeginImageContextWithOptions(self.bounds.size, NO, 0.0);
    [newImage drawInRect:CGRectMake(0, 0, self.bounds.size.width,self.bounds.size.height)];
    ...
    CGFloat dy = arrowDirection == MAMapSMCalloutArrowDirectionUp ? TOP_ANCHOR_MARGIN : 0;
...
//
@implementation MAMapSMCalloutBackgroundView

+ (MAMapSMCalloutBackgroundView *)systemBackgroundView {
    MAMapSMCalloutImageBackgroundView *background = [MAMapSMCalloutImageBackgroundView new];
    background.leftCapImage = [[self embeddedImageNamed:@"AMapSMCalloutViewLeftCap"] stretchableImageWithLeftCapWidth:16 topCapHeight:20];
    background.rightCapImage = [[self embeddedImageNamed:@"AMapSMCalloutViewRightCap"] stretchableImageWithLeftCapWidth:0 topCapHeight:20];
    background.topAnchorImage = [[self embeddedImageNamed:@"AMapSMCalloutViewTopAnchor"] stretchableImageWithLeftCapWidth:0 topCapHeight:33];
    background.bottomAnchorImage = [[self embeddedImageNamed:@"AMapSMCalloutViewBottomAnchor"] stretchableImageWithLeftCapWidth:0 topCapHeight:20];
    background.backgroundImage = [[self embeddedImageNamed:@"AMapSMCalloutViewBackground"] stretchableImageWithLeftCapWidth:0 topCapHeight:20];
    return background;
}
...
//
@implementation MAMapSMCalloutImageBackgroundView {
    UIImageView *leftCap, *rightCap, *topAnchor, *bottomAnchor, *leftBackground, *rightBackground;
}
...
//
@implementation MAMapSMCalloutDrawnBackgroundView
